...
voosMensais <- flights %>% group_by(month) %>% summarise(numVoos = n()) 
ggplot(voosMensais,aes(x = month,y = numVoos)) + geom_line()


# Agrupa os voos por mês e por empresa, calcula o número de voos por mês, resultando em 1 linha por grupo
voosMensaisPorEmpresa <- flights %>% group_by(carrier,month) %>% summarise(numVoos = n()) 
ggplot(voosMensaisPorEmpresa,aes(x = month,y = numVoos,color = carrier)) + geom_line()
...
ggplot(voosMensaisPorEmpresa,aes(x = month,y = numVoos,color = name)) + geom_line()
...
voosDiariosEAtrasos <- voos %>% group_by(Data,origin) %>% summarise(dep_delay = mean(dep_delay,na.rm = T),departures = n()) 
...
ggplot(voosDiariosEAtrasos,aes(x = departures,y = dep_delay,color = origin))+
  geom_point()+facet_wrap(~origin,nrow = 3)+ geom_smooth(method='lm', formula= y~x)
...
voosDiariosEAtrasosRegressao <- voosDiariosEAtrasos %>% group_by(origin) %>% summarise(coefsRegressao(dep_delay,departures))

# Agrupa voos por hora e por aeroporto, e conta o número de voos em cada hora
voosPorHoraEAtrasos <- voos %>% group_by(time_hour  ,origin) %>%
  summarise(dep_delay = mean(dep_delay,na.rm = T),departures = n()) 

# gráfico do atraso em função do número de voos em cada hora
ggplot(voosPorHoraEAtrasos,aes(x = departures,y = dep_delay,color = origin))+ geom_point()+
  facet_wrap(~origin,nrow = 3)+ geom_smooth(method='lm', formula= y~x)


# guarda os coeficientes de regressão atraso por voos na hora
voosPorHoraEAtrasosRegressao <- voosPorHoraEAtrasos %>% group_by(origin) %>% summarise(coefsRegressao(dep_delay,departures)) 
...
voos <- voos %>% group_by(Data,origin) %>% mutate(departuresPerDay = n(), precipDay = sum(precip,na.rm = T)) %>% ungroup() %>% 
  group_by(time_hour,origin) %>% mutate(departuresPerHour = n()) %>% ungroup() 


# Sem o group_by,  o que acontece com os valores de departuresPerDay , precipDay ?
voosErroDeAgrupamento <- voos %>% mutate(departuresPerDay = n(), precipDay = sum(precip,na.rm = T))  %>% 
  group_by(time_hour,origin) %>% mutate(departuresPerHour = n()) %>% ungroup()
...
ggplot(voos,aes(x= precipDay2, y = dep_delay)) + geom_boxplot()+ coord_cartesian(ylim = c(-10,50))


# Scatterplot com regressão - gráfico lento pq tem um ponto por voo
ggplot(voos, aes(x= precip, y = dep_delay, color = origin ))  + geom_point() +
  facet_wrap(~origin,nrow = 3)+ geom_smooth(method='lm', formula= y~x)


# Scatterplot com regressão - gráfico lento pq tem um ponto por voo
ggplot(voos, aes(x= seats, y = dep_delay))  + geom_point() +
facet_wrap(~origin,nrow = 3)+ geom_smooth(method='lm', formula= y~x)
...
voos <-  voos %>% group_by(hour) %>% mutate(meanDepDelay = mean(dep_delay,na.rm = T)) %>% ungroup()

# Boxplot atraso vs hora do dia
ggplot(voos,aes(x = factor(hour),y = dep_delay,fill = meanDepDelay)) + geom_boxplot() + coord_cartesian(ylim = c(-10,50))

# Boxplot atraso vs mes
ggplot(voos,aes(x = factor(month),y = dep_delay)) + geom_boxplot() + coord_cartesian(ylim = c(-10,50))
...
pioresVoosdeCadaDia <- voos %>% group_by(Data,origin) %>% filter(dep_delay >= quantile(dep_delay,.99,na.rm = T)) %>% ungroup()
